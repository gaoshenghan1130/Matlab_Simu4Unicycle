...
% Prevent page stretching that can cause overfull vbox warnings
\raggedbottom
...
\begin{table}[htbp]
  ...
  \begin{tabular}{
    ...
    &
    $220~\mathrm{mm}$ total, corresponding to approximately $\pm110~\mathrm{mm}$ from the center
    &
    The moving mass cannot continue producing corrective motion after reaching an end stop. The firmware begins limiting outward force when $|r|>0.10~\mathrm{m}$.
    \\
    ...
    &
    $2.0^\circ$ nominal static error and $3.5^\circ$ nominal dynamic error; approximately $5^\circ$--$10^\circ$ deviation observed experimentally
    &
    The orientation error is comparable to the allowable lean-angle range and can cause an incorrect control response.
    \\

    BNO085 angular-velocity accuracy
    &
    ...
    $10~\mathrm{ms}$
    &
    The control input is updated discretely rather than continuously, further increasing the effective feedback delay.
    \\
