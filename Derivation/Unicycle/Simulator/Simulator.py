...
class Simulator:
    ...
    def run(self, x0, t_span, t_eval, controller=None, x0_2=None, t_span_2=None, t_eval_2=None, controller_2=None):
        if controller is None:
            controller = lambda t, s: {'T_W': 0.0, 'F_L': 0.0}

        sol = solve_ivp(
            fun=self.model.get_equations_of_motion,
            t_span=t_span,
            y0=x0,
            t_eval=t_eval,
            args=(controller,),
        ...
        self.sol = sol
        if self.compare: # solve again for the second simulation if we want to compare the results
            if x0_2 is None or t_span_2 is None or t_eval_2 is None:
                raise ValueError("For comparison, x0_2, t_span_2, and t_eval_2 must be provided.")
            if controller_2 is None:
                controller_2 = lambda t, s: {'T_W': 0.0, 'F_L': 0.0}
            sol2 = solve_ivp(
                fun=self.model2.get_equations_of_motion,
                t_span=t_span_2,    
                y0=x0_2,
                t_eval=t_eval_2,
                args=(controller_2,),
    ...
    def compare_plot(self): # plot solution 1 and 2 together for comparison
        ...
        # (psi, theta, phi, gamma)
        for i in [0, 1, 2, 4]:
            axes[0].plot(self.sol.t, self.sol.y[i], label=f'Simulation 1 - {self.state_labels[i]}')
            axes[0].plot(self.sol2.t, self.sol2.y[i], '--', label=f'Simulation 2 - {self.state_labels[i]}')
        axes[0].set_ylabel('Angle (rad)')
        axes[0].legend()
        axes[0].grid(True, alpha=0.3)
        ...
        # (r)
        axes[1].plot(self.sol.t, self.sol.y[3], label=f'Simulation 1 - {self.state_labels[3]}', color='red')
        axes[1].plot(self.sol2.t, self.sol2.y[3], '--', label=f'Simulation 2 - {self.state_labels[3]}', color='orange')
        axes[1].set_ylabel('Displacement (m)')
        axes[1].set_xlabel('Time (s)')
        axes[1].legend()
        axes[1].grid(True, alpha=0.3)
