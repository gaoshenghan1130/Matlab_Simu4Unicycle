from Models import GetDynamics
import numpy as np
...
class UnicycleModel:
    ...
    def load_dynamics(self, model_id : MODEL_T, recalculate=False):
        if recalculate:
            self.M, self.f = GetDynamics(model_id)
    ...
    def get_equations_of_motion(self, t, state, u_input, params):
        ...
        if self.M is None or self.f is None:
            raise ValueError("Mass matrix and forcing function must be loaded before getting equations of motion. Please call load_dynamics() first.")

        M_eval = self.M(*args)
        f_eval = self.f(*args)

        return np.linalg.solve(M_eval, f_eval).flatten() # inversion of M to get u' = M^-1 * f
