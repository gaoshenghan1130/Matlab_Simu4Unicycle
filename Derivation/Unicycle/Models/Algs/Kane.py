...
class KaneDerivation(Derivation):
    ...
    def store_dynamics(self):
        ...
        if self.mass_matrix is None or self.forcing is None:
            raise ValueError("Mass matrix and forcing must be derived before storing dynamics.")

        args = [self.q1, self.q2, self.q3, self.q4, self.q5, self.u1, self.u2, self.u3, self.u4, self.u5, dynamicsymbols("T_W"), dynamicsymbols("F_L"), symbols("R"), symbols("h"), symbols("m"), symbols("m_w"), symbols("m_l"), symbols("g")]

        mass_matrix_func = lambdify(args, self.mass_matrix, "numpy")
        forcing_func = lambdify(args, self.forcing, "numpy")

        with open(f"/BinModels/{self.filename}.pkl", "wb") as f:
    ...
    def derive(self, get_forcing=False, display_result=False):
        ...
        E2 = E1.orientnew("E2", "Axis", [self.q2, E1.x])  # rotate around x-axis by theta
        E3 = E2.orientnew("E3", "Axis", [self.q5, E2.y])  # rotate around y-axis by gamma
        EW = E3.orientnew(
        ...
        w_total = E2.ang_vel_in(E0) + self.u3 * E2.y  # Total angular velocity fo the wheel
...
kaneDerivation = KaneDerivation()
__all__ = ["kaneDerivation"]
